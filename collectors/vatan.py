...
def parse_product_page(product_url, html):
    ...
    return {"merchant":"Vatan","merchant_product_id":sku,"brand":brand_name(product),"title":title,"price":price,"image_url":image_url(product),"product_url":canonical_url(product_url),"in_stock":"outofstock" not in availability,"gtin":clean_gtin(product)}
...
def get_or_create_merchant():
    ...
    rows=sb("POST","merchants",body={"name":"Vatan","slug":"vatan","domain":"vatanbilgisayar.com","active":True},prefer="return=representation")
    return rows[0]["id"]
...
def create_variant(item):
    ...
    rows=sb("POST","product_variants",body={"product_id":product_id,"gtin":item.get("gtin"),"sku":f"vatan:{item['merchant_product_id']}","image_url":item.get("image_url"),"active":True},prefer="return=representation")
    return rows[0]["id"]
...
def save_products_to_supabase(products):
    ...
    print(f"Supabase: {saved} Vatan ürün/fiyat kaydı işlendi."); return saved
