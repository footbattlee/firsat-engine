...
SEARCH_URL = BASE_URL + "/arama/{query}/"
DEFAULT_QUERY = os.getenv("VATAN_QUERY", "termos matara").strip() or "termos matara"
LIMIT = int(os.getenv("VATAN_LIMIT", "20"))
REQUEST_TIMEOUT = 30
REQUEST_DELAY_SECONDS = float(os.getenv("VATAN_REQUEST_DELAY", "0.35"))
REQUEST_HEADERS = {
...
CATEGORY_URLS = {
    "termos": BASE_URL + "/termoslar/",
    "matara": BASE_URL + "/termoslar/",
}
...
def resolve_search_url(query):
    tokens = normalize_text(query).split()
    for token in tokens:
        if token in CATEGORY_URLS:
            return CATEGORY_URLS[token]
    return SEARCH_URL.format(query=quote_plus(query).replace("+", "%20"))
...
def clean_gtin(product):
    for key in ("gtin14", "gtin13", "gtin12", "gtin8", "gtin"):
        ...
        if m and len(m.group()) in (8, 12, 13, 14):
            return m.group()
    return None
...
def collect(query=DEFAULT_QUERY, limit=LIMIT):
    ...
    search_url = resolve_search_url(query)
    ...
    for i, url in enumerate(urls, 1):
        ...
        try:
            ...
            if not title_matches_query(item["title"], query):
                print(f"[{i}/{len(urls)}] IRRELEVANT: {item['title'][:80]}")
                continue
            if item["merchant_product_id"] in seen_skus:
                print(f"[{i}/{len(urls)}] DUPLICATE SKU: {item['merchant_product_id']}")
                continue
            seen_skus.add(item["merchant_product_id"])
...
def get_or_create_merchant():
    rows = sb("GET", "merchants", {"slug": "eq.vatan", "select": "id", "limit": "1"})
    if rows:
        return rows[0]["id"]
    rows = sb(
        "POST",
        "merchants",
        body={"name": "Vatan", "slug": "vatan", "domain": "vatanbilgisayar.com", "active": True},
...
def find_offer(merchant_id, mpid):
    rows = sb(
        "GET",
        "offers",
        {"merchant_id": f"eq.{merchant_id}", "merchant_product_id": f"eq.{mpid}", "select": "id", "limit": "1"},
    )
    return rows[0] if rows else None


def create_variant(item):
    slug = f"vatan-{re.sub(r'[^a-z0-9]+', '-', item['merchant_product_id'].lower()).strip('-')}"
    rows = sb(
        "POST",
        "products",
        {"on_conflict": "slug"},
    ...
    rows = sb(
        ...
        body={
            ...
            "gtin": item.get("gtin"),
            "sku": f"vatan:{item['merchant_product_id']}",
            "image_url": item.get("image_url"),
            "active": True,
        },
        prefer="return=representation",
...
def save_products_to_supabase(products):
    ...
    for item in products:
        ...
        saved += 1
    print(f"Supabase: {saved} Vatan ürün/fiyat kaydı işlendi.")
    return saved


def main():
    products = collect()
    print(json.dumps({"ok": bool(products), "source": "vatan", "query": DEFAULT_QUERY, "count": len(products), "products": products}, ensure_ascii=False, indent=2))
    if products:
