...
DEFAULT_QUERY = os.getenv("N11_QUERY", "termos matara").strip() or "termos matara"
...
def extract_product_urls(html, limit):
    ...
    for a in soup.select("a[href]"):
        ...
        if "/urun/" not in url or "n11.com" not in url:
            continue
        if url in seen:
            continue
        seen.add(url)
...
def parse_product_page(product_url, html):
    ...
    if not sku:
        m = re.search(r"-(\d+)$", urlparse(product_url).path)
...
def get_or_create_merchant():
    rows = sb("GET", "merchants", {"slug": "eq.n11", "select": "id", "limit": "1"})
    if rows: return rows[0]["id"]
    rows = sb("POST", "merchants", body={"name": "n11", "slug": "n11", "domain": "n11.com", "active": True}, prefer="return=representation")
    return rows[0]["id"]
...
def create_variant(item):
    slug = f"n11-{re.sub(r'[^a-z0-9]+', '-', item['merchant_product_id'].lower()).strip('-')}"
    ...
    rows = sb("POST", "product_variants", body={"product_id": product_id, "gtin": item.get("gtin"), "sku": f"n11:{item['merchant_product_id']}", "image_url": item.get("image_url"), "active": True}, prefer="return=representation")
    return rows[0]["id"]
...
def save_products_to_supabase(products):
    ...
    for item in products:
        ...
        if offer:
            ...
            offer_id = rows[0]["id"]
        sb("POST", "price_history", body={"offer_id": offer_id, "price": item["price"], "in_stock": item["in_stock"], "checked_at": checked_at}, prefer="return=minimal")
        saved += 1
    print(f"Supabase: {saved} n11 ürün/fiyat kaydı işlendi.")
    return saved


def main():
    products = collect()
    print(json.dumps({"ok": bool(products), "source": "n11", "query": DEFAULT_QUERY, "count": len(products), "products": products}, ensure_ascii=False, indent=2))
